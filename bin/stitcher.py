...
def get_slices(arr: np.ndarray, hor_f: int, hor_t: int, ver_f: int, ver_t: int, padding: dict, overlap=0):
    ...
    block_slice = (
    slice(top_pad_size + overlap, block_shape[0] + overlap), slice(left_pad_size + overlap, block_shape[1] + overlap))
...
def get_remapping_of_border_labels(path_list: List[Path],
    ...
    for i in range(0, y_nblocks):
        for j in range(0, x_nblocks - 1):
            ...
            htask.append(
                dask.delayed(_get_map_of_overlapping_labels)(path_list, img1_id, img2h_id, overlap, 'horizontal'))
    ...
    for i in range(0, y_nblocks - 1):
        for j in range(0, x_nblocks):
            ...
            vtask.append(
                dask.delayed(_get_map_of_overlapping_labels)(path_list, img1_id, img2v_id, overlap, 'vertical'))
...
def main(img_dir: Path, out_path: Path, overlap: int, padding_str: str, is_mask: bool):
    ...
    with tif.TiffWriter(path_to_str(out_path), bigtiff=True) as TW:
        for p in range(0, npages):
            print('\npage', p)
            print('stitching')
            plane, tile_additions = stitch_plane(path_list, p, x_nblocks, y_nblocks, block_shape, dtype, overlap,
                                                 padding, remap_dict)
            if remap_dict is not None:
